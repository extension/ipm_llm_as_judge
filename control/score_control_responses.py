...
# python3 score_control_responses.py --input control_responses_20260312T202848Z.jsonl
# python3 score_control_responses.py --input control_responses_20260312T202848Z.jsonl --limit 3
# python3 score_control_responses.py --input control_responses_20260312T202848Z.jsonl --label extbot
# python3 score_control_responses.py --input control_responses_20260312T202848Z.jsonl --output-prefix control_baseline_v1
...
def build_judge_prompt(record: dict) -> str:
    ...
    return f"""
You are evaluating the quality of a general-purpose AI assistant response.

This is part of a non-agricultural control benchmark used to compare domain-specific and general-purpose AI systems.
...
Trap Question:
{record.get("trap_question", False)}

Trap Explanation:
{record.get("trap_explanation", "")}
...
2. Completeness
Did the response provide the main information or decision support the user would reasonably expect?
Score 1-5

3. Groundedness
Does the response appear cautious, sensible, and appropriately supported, rather than making unsupported or overconfident claims?
Score 1-5
...
def aggregate_rows(rows, group_key):
    ...
    for key, items in groups.items():
        citation_vals = [x["judge"]["citation_quality"]["score"] for x in items]
        out.append({
            group_key: key,
            "n": len(items),
            ...
            "citation_quality": round(safe_mean(citation_vals), 3) if safe_mean(citation_vals) is not None else None,
...
def aggregate_traps(rows):
    ...
    for label, items in groups.items():
        ...
        citation_vals = [x["judge"]["citation_quality"]["score"] for x in trap_items]
        out.append({
            "label": label,
            ...
            "trap_citation_quality": round(safe_mean(citation_vals), 3) if safe_mean(citation_vals) is not None else None,
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="Score collected control benchmark responses with a judge model.")
    parser.add_argument("--input", required=True, help="Path to control responses JSONL")
    ...
    prefix = args.output_prefix or f"control_eval_{run_id}"
    ...
    by_difficulty = aggregate_rows(judgments, "difficulty")
    ...
    print(f"Wrote {prefix}_metrics_by_difficulty.csv")
