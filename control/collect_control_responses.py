# python3 collect_control_responses.py --limit 3
...
# Config
# -----------------------------
DATASET_PATH = "control_question_set.csv"
...
SYSTEM_PROMPT = (
    "You are a helpful general-purpose assistant answering everyday non-agricultural questions. "
    "Be concise but complete. Do not browse the web. Do not invent citations. "
    "If uncertain, say so."
)

HTTP_REFERER = os.environ.get("OPENROUTER_HTTP_REFERER", "")
X_TITLE = os.environ.get("OPENROUTER_X_TITLE", "Control Eval Collector")
...
def select_rows(rows: list[dict], offset: int = 0, limit: int | None = None, question_id: str | None = None) -> list[dict]:
    if question_id is not None:
        return [r for r in rows if str(r.get("id")) == str(question_id)]
...
def main():
    parser = argparse.ArgumentParser(
        description="Collect benchmark responses from ExtBot and OpenRouter models for the control question set."
    )
    parser.add_argument("--dataset", default=DATASET_PATH, help="Path to control benchmark CSV")
    ...
    output_path = args.output or f"control_responses_{run_id}.jsonl"
    ...
    with open(output_path, "w", encoding="utf-8") as out:
        for row in rows:
            ...
            for target in targets:
                record = {
                    "run_id": run_id,
                    "benchmark_type": "control",
                    "timestamp": utc_now(),
                    "question_id": row.get("id"),
                    "category": clean_optional(row.get("category")),
                    "difficulty": clean_optional(row.get("difficulty")),
                    "trap_question": parse_bool(row.get("trap_question")),
                    "trap_explanation": clean_optional(row.get("trap_explanation")),
